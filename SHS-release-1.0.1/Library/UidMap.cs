...
namespace SHS {
  public class UidMap {
    ...
    public UidMap(long[] items, bool val)  // Khoi insert this function. Remove uid if it equal -1
    {
        int count = 0;
        ...
        this.vals = new long[count];
        count = 0;
        for (int i = 0; i < items.Length; i++)
        {
            if (items[i] > -1) this.vals[count++] = items[i];
        }
        this.pos = this.vals.Length;
        this.sorted = false;
    }
