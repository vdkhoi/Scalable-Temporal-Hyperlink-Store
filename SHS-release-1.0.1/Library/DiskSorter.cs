...
namespace SHS {
  ...
  public class DiskSorter<T> : IDisposable {
    ...
    public DiskSorter(IComparer<T> compare, Writer write, Reader read, int bufSz) {
	  ...
	  //System.Console.WriteLine("Khoi: Starting memory allocation " + bufSz);
      ...
      int i = 0;
	  while (this.buffer == null && i++ < 10) this.buffer = new T[bufSz];
	  
      this.tmpFiles = new List<string>();
      i = 0;
	  while (this.tmpFiles == null && i++ < 10) this.tmpFiles = new List<string>();
	  ...
	  //if (this.buffer != null)
      //  System.Console.WriteLine("Khoi: Success memory allocation");
      //else
	  //  System.Console.WriteLine("Khoi: Fail memory allocation");
    }
    ...
    public void Add(T item) {
	  //if (this.buffer == null)
      //	System.Console.WriteLine("Khoi: Cannot add item since buffer is null");
