...
namespace SHS
{
    ...
    public class Store
    {
        ...
        public int AddIndividualTempLinks(IEnumerator<TempPairLinks> pageTempLinks,
                            IEnumerator<PageURL> pageURL,
                            int urlCellStrideLength = 32,
                            int fwdCellStrideLength = 32,
                            int bwdCellStrideLength = 32,
                            LinkCompression fwdCellCompression = LinkCompression.VarNybble,
                            LinkCompression bwdCellCompression = LinkCompression.VarNybble)
        {
            ...
            while (true)
            {
                try
                {
                    ...
                    foreach (var ch in this.channels)
                    {
                        ch.WriteUInt32((uint)OpCodes.AddTemporalPageLinks);
                        ch.WriteInt32(urlCellStrideLength);
                        ch.WriteInt32(fwdCellStrideLength);
                        ch.WriteInt32(bwdCellStrideLength);
                        ch.WriteInt32((int)fwdCellCompression);
                    ...
                    while (pageURL.MoveNext())
                    {
                        pageCnt++;
                        this.channels[ping.PartitionID(pageURL.Current.pageUrl)].WriteString(pageURL.Current.pageUrl);
                        lastURL = pageURL.Current.pageUrl;
                    }
                    ...
                    //Console.WriteLine("Stop to count URL");
                    ...
                    long dupLinkCnt = 0;
                    var smt = new ThriftySetMemberTest();
                    while (pageTempLinks.MoveNext())
                    {
                        var pl = pageTempLinks.Current;
                        pageCnt++;
                        
                        if (smt.ContainsOrAdd(pl.pageSrcUrl + " " + pl.pageDstUrl))
                        {
                            dupPageCnt++;     // Need to tell the server number of outlink to receive finite number of node URL
                            dupLinkCnt++;
                        }
                        else
                        {
                            var ch = this.channels[ping.PartitionID(pl.pageSrcUrl)];
                            ch.WriteString(pl.pageSrcUrl);
                            ch.WriteString(pl.timeStamp);
                            ch.WriteString(pl.pageDstUrl);
                            linkCnt ++;
                        }
                    }

                    //Console.WriteLine("Stop to count link");
                    ...
                    {
                        int numPairsInRound = 0;
                        var buffers = new List<Tuple<long, long, long>>[this.channels.Length];
                        for (int i = 0; i < buffers.Length; i++)
                        {
                            buffers[i] = new List<Tuple<long, long, long>>();
                        }
                        foreach (var ch in this.channels)
                        {
                            int n = ch.ReadInt32();
                            ...
                            {
                                long uid1 = ch.ReadInt64();  // specifies receiving server
                                long timeStamp = ch.ReadInt64();  //timeStamp of uid1
                                long uid2 = ch.ReadInt64();  // may have deleted-bit
                                buffers[ping.PartitionID(uid1)].Add(new Tuple<long, long, long>(uid1, timeStamp, uid2));
                            }
                        }
                        if (numPairsInRound == 0)
                        {
                        ...
                        else
                        {
                            for (int i = 0; i < channels.Length; i++)
                            {
                                ...
                                foreach (var tup in buffer)
                                {
                                    channels[i].WriteInt64(tup.Item1);
                                    channels[i].WriteInt64(tup.Item2);
                                    channels[i].WriteInt64(tup.Item3);
                                }
                                buffer.Clear();
                                channels[i].Flush();
                            }
            ...
            while (true)
            {
                try
                {
                    ...
                    while (pageLinks.MoveNext())
                    {
                        var pl = pageLinks.Current;
                        pageCnt++;
                        this.channels[ping.PartitionID(pl.pageSrcUrl)].WriteString(pl.pageSrcUrl);
                        linkCnt++;
                    ...
                    while (pageLinks.MoveNext())
                    {
                        ...
                        {
                            var ch = this.channels[ping.PartitionID(pageSrcUrl)];
                            ch.WriteString(pageSrcUrl);
                            ch.WriteInt32(numLinks);
                            linkCnt += numLinks;

                            ch.WriteString(pageLinks.Current.pageDstUrl);  // Write for a destination
                            for (numLnk = 1; numLnk < numLinks; numLnk++)
                            {
                                pageLinks.MoveNext();
                                ch.WriteString(pageLinks.Current.pageDstUrl);
                            }
                        }
                    }
        ...
        public long UrlToUid(string url, ref int desiredEpoch)
        {
            ...
            //System.Console.WriteLine("URL: " + url + ". Epoche: " + desiredEpoch);
            while (true)
            {
                try
                {
        ...
        public long[] GetTemporalLinks(long uid, long firstTime, long lastTime, Dir dir)
        {
            return this.GetTemporalLinks(uid, firstTime, lastTime, dir, ref this.desiredEpoch);
        }

        public long[] GetTemporalLinks(long uid, long firstTime, long lastTime, Dir dir, ref int desiredEpoch)
        {
            return this.SampleTemporalLinks(uid, firstTime, lastTime, dir, -1, false, ref desiredEpoch);
        }
        ...
        public long[] SampleTemporalLinks(long uid, long firstTime, long lastTime, Dir dir, int numSamples, bool consistent)
        {
            return this.SampleTemporalLinks(uid, firstTime, lastTime, dir, numSamples, consistent, ref this.desiredEpoch);
        }
        ...
        public long[] SampleLinks(long uid, Dir dir, int numSamples, bool consistent, ref int desiredEpoch)
        {
            ...
            while (true)
            {
                try
                {
                    ...
                    //long get_parttion = sw.ElapsedMilliseconds;  // Khoi insert
                    ...
                    //long communication = sw.ElapsedMilliseconds;// Khoi insert
                    if (desiredEpoch != -1 && desiredEpoch != currentEpoch) throw new EpochPassed();
                    desiredEpoch = currentEpoch;

                    //Console.Error.WriteLine("Get partition {0} millisecoonds, get links {1} milliseconds.", get_parttion, communication - get_parttion); // Khoi insert
        ...
        public long[] SampleTemporalLinks(long uid, long firstTime, long lastTime, Dir dir, int numSamples, bool consistent, ref int desiredEpoch)
        {
            if (this.isClosed) throw new Exception("Store already closed");
            if (numSamples < -1)
            {
            ...
            while (true)
            {
                try
                {
                    ...
                    //long get_parttion = sw.ElapsedMilliseconds;  // Khoi insert
                    var ch = this.channels[id];
                    ch.WriteUInt32((uint)OpCodes.SampleTemporalLinks);
                    ch.WriteInt32((int)dir);
                    ch.WriteInt32(numSamples);
                    ch.WriteInt32(consistent ? 1 : 0);
                    ch.WriteInt64(uid);
                    ch.WriteInt64(firstTime);
                    ch.WriteInt64(lastTime);
                    ch.Flush();
                    int currentEpoch = ch.ReadInt32();
                    int n = ch.ReadInt32();
                    if (n == -1)
        ...
        public long[][] BatchedSampleLinks(long[] uids, Dir dir, int numSamples, bool consistent, ref int desiredEpoch)
        {
            ...
            while (true)
            {
                try
                {
                    //Console.Error.WriteLine("Point 1");
                    int[] chIds = new int[uids.Length];
                    //Console.Error.WriteLine("Point 1.1: {0}", uids.Length);
                    int[] chCnt = new int[channels.Length];  // All 0 by default
                    //Console.Error.WriteLine("Point 1.2: {0}", channels.Length);
                    for (int i = 0; i < uids.Length; i++)
                    {
                        int chId = ping.PartitionID(uids[i]);
                        //Console.Error.WriteLine("Point 1.3: {0}, {1}", chId, uids[i]);
                        chIds[i] = chId;
                        chCnt[chId]++;
                        //Console.Error.WriteLine("Point 1.4");
                    }
                    //Console.Error.WriteLine("Point 2");
                    for (int chId = 0; chId < channels.Length; chId++)
                    {
                        if (chCnt[chId] > 0)
                        {
                    ...
                    //Console.Error.WriteLine("Point 3");
                    for (int i = 0; i < uids.Length; i++)
                    {
                        channels[chIds[i]].WriteInt64(uids[i]);
                    }
                    //Console.Error.WriteLine("Point 4");
                    for (int chId = 0; chId < channels.Length; chId++)
                    {
                        if (chCnt[chId] > 0)
                        {
                    ...
                    bool epochPassed = false;
                    //Console.Error.WriteLine("Point 5");
                    for (int chId = 0; chId < channels.Length; chId++)
                    {
                        if (chCnt[chId] > 0)
                        {
                    ...
                    //Console.Error.WriteLine("Point 6");
                    for (int i = 0; i < uids.Length; i++)
                    {
                        int chId = chIds[i];
